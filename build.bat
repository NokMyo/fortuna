...
link /nologo /nodefaultlib /machine:x64 /subsystem:windows,6.02 /entry:WinMainCRTStartup /dynamicbase /highentropyva /nxcompat /Brepro /map:build\fortuna.map /out:build\FebiusFortuna.exe build\fortuna.obj build\app.res engine\build\FortunaOracleStatic.lib user32.lib kernel32.lib gdi32.lib comdlg32.lib shell32.lib comctl32.lib advapi32.lib winhttp.lib crypt32.lib || exit /b 1
exit /b 0
