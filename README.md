# Febius Fortuna 1.3.0

**Windows 클래식 로또 6/45 번호 생성기 · ORACLE Field Architecture**

[Windows 릴리스](https://github.com/NokMyo/fortuna/releases/tag/v1.3.0) · [사용 설명서](docs/USER_GUIDE.md) · [실제 구현 명세](engine/docs/IMPLEMENTATION.md)

**엔진 전용 폴더:** [`engine/`](engine/)에 Fortuna ORACLE Engine 1.1.0의 소스·문서·테스트·SDK를 모았습니다. 앱은 공개 API로 연결하며, 빌드할 때 엔진을 실행 파일에 포함합니다. [SDK와 호출 규격](engine/docs/ENGINE_API.md)
...
**분석 → 정밀 연구 분석**에 여섯 확장을 추가했습니다. 45C6 전체 정규화 공동 확률모형, 단계별 경험적 베이지안 축소, 후보 누락 검사와 8,192개 확장, 12개 성분의 완전한 제거 재검증, 중첩 시간순 로그 손실 평가와 사전 예측 기록, 적응적 불확실성 분석입니다.

공동 모형 번호는 **공동 모형 번호 생성 (연구용)**에서 생성합니다. 기존 번호 생성은 기존 검증 정책을 사용합니다. 연구 모형의 실제 예측 우위는 아직 입증되지 않았습니다. 연구는 계산량이 크며 취소할 수 있습니다. [수식·경계·사용법](engine/docs/RESEARCH.md)
...
**제품 버전 1.3.0은 설계 문서의 모든 장기 연구 항목 완료를 의미하지 않습니다.** 자동 데이터 공급, 고차 모형, AVX/GPU 가속, 외부 타임스탬프 등 미구현 항목은 구현 명세에 명시합니다.
