...
**엔진 전용 폴더:** [`engine/`](engine/)에 Fortuna ORACLE Engine 1.7.0의 소스·문서·테스트·SDK를 모았습니다. 앱은 공개 API로 연결하며, 빌드할 때 엔진을 실행 파일에 포함합니다. [SDK와 호출 규격](engine/docs/ENGINE_API.md)
...
Windows 10 1607 이상 또는 Windows 11, x64 환경에서 `FebiusFortuna.exe`를 실행합니다. **`FebiusFortuna.exe` 하나로 실행하며 별도 엔진 DLL이 필요 없습니다.** 설치나 관리자 권한, 별도 런타임이 필요 없습니다. **1~1242회차 추첨 이력이 EXE에 기본 내장**되어 있으며 ZIP에는 설명서와 가상 테스트 데이터도 포함됩니다.

1. 실행하면 **1~1242회차 기본 추첨 이력**이 자동으로 로드됩니다.
2. **1·5·10게임**을 선택하고 **예상 번호 추첨**을 누르면 이력과 무관한 균등 무작위 추첨을 합니다.
3. **분석 범위**에서 실제로 사용할 과거 회차의 시작·종료를 정합니다. 기본값은 내장 데이터 전체인 1~1242이며 최소 60개의 연속 회차를 선택해야 합니다. 예를 들어 1~432를 선택하면 433회차 이후 데이터는 일반·심층 ORACLE의 fitting과 검증에서 완전히 제외되고 다음 분석 대상은 433회차가 됩니다.
4. 선택 범위 내부에서는 첫 60회를 초기 학습에 사용하고 이후 회차를 과거만 보는 walk-forward 방식으로 검증합니다. 충분히 긴 범위에서는 마지막 30개 검증 대상을 독립 확인으로 유보합니다. 화면의 예상 소요시간은 CPU 마이크로벤치마크와 선택 범위를 이용해 계산합니다.
5. 같은 ORACLE 버튼을 다시 누르면 동일 데이터·동일 백테스트 범위의 완료된 분석을 재사용합니다. 필요하면 **외부 CSV**로 현재 세션의 이력을 교체할 수 있습니다.
...
검증은 회차 순서대로 과거만 학습하는 rolling/prefix 방식입니다. GUI 범위는 검증 fold만 자르는 옵션이 아니라 엔진이 실제로 보는 이력 자체를 제한합니다. 따라서 선택 종료 회차 뒤의 데이터는 최종 fitting에도 들어가지 않습니다. 균등 무작위 기준선, 모델별 BH 다중검정, 상관관계 중복 억제와 확인 구간을 통과해야 가중 정책을 승격합니다. 근거가 부족하면 균등 무작위 생성으로 돌아갑니다.
...
- [FORMAT.md](engine/data/FORMAT.md): 외부 입력 데이터 계약
- [EMBEDDED_HISTORY.md](docs/EMBEDDED_HISTORY.md): 1~1242회차 기본 내장 데이터의 원본·무결성·빌드 방식

**제품 버전 1.10.0은 설계 문서의 모든 장기 연구 항목 완료를 의미하지 않습니다.** 자동 데이터 공급, 외부 공증, GPU 가속, 통합 분포의 영구 기록, 연속 전체 사후분포 적분과 증명된 구간 연산은 포함하지 않습니다. 열 기능의 실제 구현 범위는 통합 명세에 명시합니다.
