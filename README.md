...
**엔진 전용 폴더:** [`engine/`](engine/)에 Fortuna ORACLE Engine 1.6.0의 소스·문서·테스트·SDK를 모았습니다. 앱은 공개 API로 연결하며, 빌드할 때 엔진을 실행 파일에 포함합니다. [SDK와 호출 규격](engine/docs/ENGINE_API.md)
...
3. **백테스트 범위**에서 평가할 시작·종료 회차를 정합니다. 각 대상 회차는 그 직전 이력만 학습하며, 선택 범위의 마지막 30회는 독립 확인 구간으로 예약됩니다. 선택 범위는 최소 60회입니다.
4. **ORACLE기반 예상 번호 추첨**은 일반 분석, **ORACLE기반 심층 분석 추첨**은 정밀 분석을 수행한 후 번호를 추첨합니다. 화면의 예상 소요시간은 시작 시 CPU 마이크로벤치마크와 선택 범위를 이용해 계산하며 실제 완료시간을 보장하지 않습니다.
...
검증은 회차 순서대로 과거만 학습하는 rolling/prefix 방식입니다. GUI에서 평가 대상 회차 범위를 직접 지정할 수 있으며, 어떤 범위를 선택해도 각 대상의 미래 행은 학습에 사용하지 않습니다. 선택 범위의 마지막 30회는 독립 확인용으로 유보됩니다. 균등 무작위 기준선, 모델별 BH 다중검정, 상관관계 중복 억제와 확인 구간을 통과해야 가중 정책을 승격합니다. 근거가 부족하면 균등 무작위 생성으로 돌아갑니다.
...
**제품 버전 1.9.0은 설계 문서의 모든 장기 연구 항목 완료를 의미하지 않습니다.** 자동 데이터 공급, 외부 공증, GPU 가속, 통합 분포의 영구 기록, 연속 전체 사후분포 적분과 증명된 구간 연산은 포함하지 않습니다. 열 기능의 실제 구현 범위는 통합 명세에 명시합니다.
