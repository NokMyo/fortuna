...
- 백테스트 UI를 실제 분석 이력 범위 선택 방식으로 변경.
- 전체 로드 원본을 별도 보존하고 선택 범위만 활성 history로 복사해 최종 fitting과 다음 회차에도 범위 적용.
- 1회차부터 범위 시작 가능, 최소 60개 연속 회차 요구.
- 선택 종료가 432회면 최종 분석 latest round도 432, target도 433이 되도록 수정.
- 사용 가능 범위 표시 추가, 내장 데이터 로드/스냅샷 및 하단 ORACLE 안내 UI 제거.
- 로그인 최초 상태는 빈칸 유지, 잘못된 입력에만 유효값 안내 표시.
