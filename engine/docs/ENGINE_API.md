# Fortuna ORACLE Engine 1.7.0 — Windows x64 SDK
...
- Application: **Febius Fortuna 1.10.0** (`FebiusFortuna.exe`).
- Engine: **Fortuna ORACLE Engine 1.7.0** (`FortunaOracle.dll`).
...
| `SetBacktestRange(first,last)` | ABI-compatible name for selecting the inclusive historical **analysis-data range**. At least 60 contiguous rows are required. The engine rebuilds its active history from the preserved full source, so final fitting, validation, reports and target round cannot read rows after `last`. `0,0` restores the full loaded source range. Changing the range invalidates analysis caches. |
