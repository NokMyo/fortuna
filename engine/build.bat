...
if /i not "%~1"=="sdk" exit /b 0
rc /nologo /c65001 /fo build\engine.res engine.rc || exit /b 1
link /nologo /dll /noentry /nodefaultlib /machine:x64 /dynamicbase /highentropyva /nxcompat /release /incremental:no /Brepro /def:FortunaOracle.def /implib:build\FortunaOracle.lib /out:build\FortunaOracle.dll build\oracle.obj build\engine.res kernel32.lib bcrypt.lib || exit /b 1
